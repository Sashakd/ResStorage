...
struct chainlist
{
    char* word;
    int cpy;
	enum {empty, filled, freed} status;
...
struct chainlist* searchop(char* word, struct chainlist *masop[256], const int* mas, int *ch)
{
    ...
    {
        (*ch)++;
        if ((strcmp(p->word, word) == 0) && (p->status != freed))
        {
            return p;
        }
        q = p;
...
// Удаление указанного элемента для открытой адресации
// Просто пометим элемент как удалённый, но физически освобождать не будем
int delop(char* word, struct chainlist *masop[256], const int* mas)
{
	int ch=0;
	struct chainlist *p = searchop(word, masop, mas, &ch);
...
int main()
{
    ...
    while (c != 0)
    {
		...
		if (c == 5)
		{
			...
			printf("\n\nЗадайте слово для удаления из : ");
			if (scanf("%s", s))
			{
				if (delop(strdup(str), masop, randm))
				{
					printf("\nЭлемент успешно удалён\n");
				}
				else
					printf("\nВНИМАНИЕ! Такого элемента нет!");
			}
			else
			{
				printf("\nОШИБКА! Неверно задано слово для удаления\n");
				return -1;
			}
		}*/
        printf("\n\n\n\n\n\n");
